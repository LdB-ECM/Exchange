...
/*-[TimerFiqSetup]----------------------------------------------------------}
. The timer fiq interrupt rate is set to the period in usec between triggers.
. Largest period is around 16 million usec (16 sec) it varies on core speed
. RETURN: TRUE if successful,  FALSE for any failure
.--------------------------------------------------------------------------*/
bool TimerFiqSetup (uint32_t period_in_us);							// Period between timer interrupts in usec
