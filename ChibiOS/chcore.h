...
#elif ARM_CORE == ARM_CORE_CORTEX_A53
#define PORT_ARCHITECTURE_ARM_CORTEXA53
#define PORT_ARCHITECTURE_NAME          "ARMv8"
#define PORT_CORE_VARIANT_NAME          "ARM Cortex-A53"
...
#define PORT_SETUP_CONTEXT(tp, wbase, wtop, pf, arg) {                      \
  ...
  (tp)->ctx.sp->x4 = (regarm_t)(pf);                                        \
  (tp)->ctx.sp->x5 = (regarm_t)(arg);                                       \
  (tp)->ctx.sp->ELR_EL1 = (regarm_t)(_port_thread_start);                   \
}
#else
#define PORT_SETUP_CONTEXT(tp, wbase, wtop, pf, arg) {                      \
  (tp)->ctx.sp = (struct port_intctx *)((uint8_t *)(wtop) -                 \
...
#else
	#if __aarch64__ == 1
	__asm volatile ("mrs	   %[p0], daif" : [p0] "=r" (sts) : );
	#else
 ...
 */
#if __aarch64__ == 1
#define ARM_I_BIT  ( 0x02 )
#define ARM_F_BIT  ( 0x01 )
#else
...
static inline bool port_irq_enabled(syssts_t sts) {
  return (sts & (syssts_t )ARM_I_BIT) == (syssts_t)0;
}
