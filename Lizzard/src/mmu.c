...
void init_page_table (void) {
	...
	if (mailbox_write(8, (uint32_t)(uintptr_t)&msg[0])) 			// Write message to mailbox
		mailbox_read(8);											// Read the status back
	...
	for (base = 0; base < msg[5]; base++) {
		...
		if (base == 0) printf("Table value = %#x\n", Stage2map1to1[base].Raw64);
	}
...
void mmu_init(void)
{
	...
	r = ((0x00ul << (MT_DEVICE_NGNRNE * 8)) | \
		 ...
		 (0xfful << (MT_NORMAL * 8)));
    __asm__ volatile ("msr mair_el1, %0" : : "r" (r));

	// Bring both tables online and execute memory barrier
	__asm__ volatile ("msr ttbr0_el1, %0" : : "r" ((uintptr_t)&Stage1map1to1[0]));
    ...
    __asm__ volatile ("msr tcr_el1, %0; isb" : : "r" (r));
