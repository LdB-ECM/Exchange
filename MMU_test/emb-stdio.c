    ...
    flags_scanned:
            ...
            if (base != 0)
            {
				...
				{ 
					...
					str[0] = '0';
					pos = 1;   // Ulong_to_string will now start at str[1]
				}
				...
				if (hashtype == 3)
				{
					...
					str[0] = '0';
					str[1] = 'x';
					pos = 2;   // Ulong_to_string will now start at str[2]
				}
				/* run conversion avoiding prefix that may have been added */
				ulong_to_string(ularg, &str[pos], base, alt_digits);
...
*/
static int sprntf(int c, void* _sptr)
{
...
static int sprntf1(int c, void* _sptr)
{
	CHAR_OUTPUT_HANDLER handler = _sptr;
	handler(c);
	return (int)c;
}
...
int printf (const char *fmt, ...)
{
	...
	count = _doprnt(fmt, args, sprntf1, Console_WriteChar);
...
int sprintf (char* buf, const char* fmt, ...)
{
	...
	s = buf;
	va_start(ap, fmt);
	_doprnt(fmt, ap, sprntf, (void*)&buf);
	va_end(ap);
	*s = '\0';
