...
#ifdef __arm__    // Compiling for raspberry Pi we nned to use linux drivers 
#include <unistd.h>				//Needed for I2C port under linux
#include <fcntl.h>				//Needed for I2C port under linux
#include <sys/ioctl.h>			//Needed for I2C port under linux
#include <linux/i2c-dev.h>		//Needed for I2C port under linux
#else
#include <Wire.h>
#endif
...
int16_t INA226::readRegister16(uint8_t reg)
{
    int16_t value;
    #ifdef __arm__
        if (ioctl(file_i2c, I2C_SLAVE, inaAddress) < 0)
        {
            printf("Failed to acquire bus access and/or talk to slave.\n");
            //ERROR HANDLING; you can check errno to see what went wrong
        }
        uint8_t buffer[2];
        if (read(file_i2c, buffer, 2) != 2)		// read() returns the number of bytes actually read
        {
...
void INA226::writeRegister16(uint8_t reg, uint16_t val)
{
    ...
    val >>= 8;
        ...
        buffer[2] = vla;
        if (write(file_i2c, buffer, 3) != 3) // write() returns the number of bytes actually written
        {
            /* ERROR HANDLING: i2c transaction failed */
